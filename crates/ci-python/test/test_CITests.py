"""Copied and modified from pgmpy."""
...
class TestDiscreteTests(unittest.TestCase):
    def setUp(self):
        self.df_adult = pd.read_csv(Path(__file__).parent / "testdata" / "adult.csv")
...
class TestResidualMethods(unittest.TestCase):
    ...
    def test_pearsonr(self):
        coef, p_value = pearsonr(
            ...
            data=self.df_indep,
            boolean=False,
            # seed=42,  # TODO: kwarg doesn't exist??
        )
        self.assertTrue(abs(coef) <= 0.1)
        self.assertTrue(p_value >= 0.04)

        # coef, p_value = pearsonr(X="X", Y="Y", Z=["Z1", "Z2", "Z3"], data=self.df_dep, boolean=False, seed=42)  # TODO: kwarg doesn't exist??
        coef, p_value = pearsonr(X="X", Y="Y", Z=["Z1", "Z2", "Z3"], data=self.df_dep, boolean=False)
        self.assertTrue(coef >= 0.1)
    ...
    def test_pearsonr_equivalence(self):
        is_independent = pearsonr_equivalence(
            ...
            boolean=True,
            significance_level=0.05,
            # delta_th=0.3,  # TODO: kwarg doesn't exist??
        )
        ...
        is_independent = pearsonr_equivalence(
            ...
            boolean=False,
            significance_level=0.05,
            # delta_th=0.5,  # TODO: kwarg doesn't exist??
        )
